...
data class Prompt(
  ...
  @CheckResult
  internal fun toPromptInfo(
    context: Context
  ) = PromptInfo.Builder().apply {
    val res = context.resources
    setTitle(res.getText(title))
    setConfirmationRequired(confirmRequired)
    setDeviceCredentialAllowed(deviceCredentialsAllowed)
    subtitle?.let { setSubtitle(res.getText(it)) }
    description?.let { setDescription(res.getText(it)) }
    negativeButtonText?.let { setNegativeButtonText(res.getText(it)) }
  }.build()
